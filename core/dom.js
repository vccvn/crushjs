import Helper, { assignValue, assignWithout, cl, date, getType, inArray, isArray, isBoolean, isEmpty, isFunction, isNull, isObject, isString, Str, _defineProperty, _instanceof } from './helper.js';
...
Dom = createClass("Dom")({
    ...
    ___assignDynamicProperties___: function(){
        ...
        function next(args){
            ...
            if(self.parent){
                var s = false;
                for (let index = 0; index < self.parent.children.length; index++) {
                    const child = self.parent.children[index];
                    if(s){
                        if(isObject(args)){
                            var a = true;
        ...
        function previous(args){
            if(self.parent){
                var s = false;
                for (let index = self.parent.children.length - 1; index > -1 ; index--) {
                    const child = self.parent.children[index];
                    if(s){
                        if(isObject(args)){
                            var a = true;
    ...
    getProp: function getProp(prop, all) {
        if (!this.el) return "";
    ...
    attrData: function attrData(key) {
            ...
            if (attrs.length) {
                for (var i = 0; i < attrs.length; i++) {
                    ...
                    if (attr.name.toLowerCase().indexOf("data-") === 0) {
                        ...
                        var b = a.split("-");
                        var c = b.shift();
                        var d = b.map(function (v) { return Helper.str.ucfirst(v) });
     ...
     * @returns {DomElement}
     */
    const$append: function append(child) {
     ...
     * @returns {Query|Dom}
     */
    const$prepend: function prepend(child) {
        if (typeof child == "undefined" || isNull(child) || simpleTags.indexOf(this.tagName) !== -1) return this;
     ...
     */
    final$setContent: function setContent(content) {
        this.removeChild();
        return this.append(content);
    },


    final$stopTransmission: function stopTransmission() {
     ...
     */
    final$receiveFromParent: function receiveFromParent(channel, data, sentId) {
        var next = true;
        var self = this;
        var a = 0;
        var stop = false;
     ...
     */
    final$__sendToChildren__: function (channel, data, sentId) {
     ...
     * @returns {DomElement}
     */
    final$onParentTransmission: function onParentTransmission(channel, handler) {
     ...
     * @returns {DomElement}
     */
    final$onParentData: function onParentData(channel, handler) {
     ...
     * @param {*} data dữ liệu bất kỳ
     */
    final$receiveFromChildren: function receiveFromChildren(channel, data) {
        var self = this;
        var next = true;
        var a = 0;
        var arr, fn, s = undefined;
     ...
     * @param {*} data dữ liệu bất kỳ
     */
    final$sendToParent: function sendToParent(channel, data) {
     ...
     * @returns {DomElement}
     */
    final$onChildrenTransmission: function onChildrenTransmission(channel, handler) {
        if (isString(channel) && isFunction(handler)) {
     ...
     * @returns {DomElement}
     */
    final$onChildrenData: function onChildrenData(channel, handler) {
     ...
     * @returns {mixed}
     */
    final$callParentMethod: function callParentMethod(method, args) {
     ...
     * @returns {mixed}
     */
    final$cpm: function cpm(method, args) {
     ...
     * @returns {mixed}
     */
    final$parentCall: function parentCall(method, args) {
        return this.callParentMethod.call(this, method, args);
    },
     ...
     * @returns {mixed}
     */
    final$callParent: function callParent(method, args) {
        return this.callParentMethod.call(this, method, args);
    },
     ...
     */
    final$onChildrenCallMethod: function onChildrenCallMethod(method, args) {
     ...
     */
    final$callChildrenMethod: function callChildrenMethod(method, args, className) {
     ...
     * @param {array} args tham so
     */
    final$ccm: function ccm(method, args, className) {
     ...
     * @param {array} args tham so
     */
    final$childrenCall: function childrenCall(method, args, className) {
        return this.callChildrenMethod(method, args, className);
    },
     ...
     * @param {array} args tham so
     */
    final$callChildren: function callChildren(method, args, className) {
        return this.callChildrenMethod(method, args, className);
    },
    ...
    final$__callChildrenMethod__: function __callChildrenMethod__(method, args, className) {
        var instanceId = this.getInstanceID();
     ...
     */
    final$onCallMethodFromParent: function onCallMethodFromParent(method, args, className) {
        var self = this;
        function f() {
            if (typeof self[method] == "function") {
                var fn = self[method];
    ...
    final$stopCallChildrenMethod: function stopCallChildrenMethod() {
     ...
     * @param {*} data du lieu
     */
    final$sendToSiblings: function sendToSiblings(slug, data) {
     ...
     * @param {*} data du lieu
     */
    final$onReceiveFromSiblings: function onReceiveFromSiblings(channel, data) {
        var self = this;
        var next = true;
        var a = 0;
        var arr, fn, s = undefined;
     ...
     * @param {*[]} args tham số
     */
    final$callSiblingMethod: function callSiblingMethod(method, args) {
        if (this.parent && this.parent.children && this.parent.children.length > 1) {
     ...
     * @param {*[]} args tham số
     */
    final$callSiblings: function callSiblings(method, args) {
     ...
     * @param {*[]} args tham số
     */
    final$siblingCall: function callSiblings(method, args) {
        return this.callSiblingMethod(method, args);
    },
     ...
     * @param {*[]} args tham số
     */
    final$csm: function callSiblings(method, args) {
        return this.callSiblingMethod(method, args);
    },
    ...
    getElementArgsData: function (args, rules) {
        ...
        for (var index = 0; index < args.length; index++) {
            ...
            if (isString(vl)) {
                ...
                if (vl.parent && isObject(vl.parent) && vl.parent.isDom) this.parent = vl.parent;
                ...
                if (index == 0) {
                    ...
                    else if (objectHasProperty(vl, dataCheckTypes)) {
                        data = {
                            nav: vl.nav,
                            tabs: vl.tabs
                        };
                    ...
                    if (isObject(vl.attrs)) {
                        assignValue(attrs, vl.attrs);
                    }
                    for (var kv in vl) {
                        if (Object.hasOwnProperty.call(vl, kv)) {
                            var v = vl[kv];
                            if (!inArray(['tabs', 'attrs', 'parent'])) {
                                if (!hasData || (hasData && kv != "data")) {
                                    attrs[kv] = v;
                                }
                            }
                        }
                    ...
                    if (isArray(vl.tabs)) tabs = vl.tabs;
                    assignWithout(attrs, vl, ['attrs', 'parent', 'tabs']);
                }
     ...
     * @returns {DomElement}
     */
    final$getRootElement: function getRootElement() {
    ...
    final$moveTo: function moveTo(parent, pos) {
    ...
    static$withParent: function withParent(parent) {
        var self = this;
        ...
        if (typeof self != "function") {
            console.error("The context of withParent Method mush be DomElement or DomElement's Children");
            return false;
        }
        var params = [];
...
function create(tag, children, attributes) {
    ...
    function addAttrValue(k, vl) {
        ...
        else if (Helper.inArray(["content", "children"], s)) {
        ...
        else if(isString(vl) && vl.substr(0, 2) == '{{' && vl.substr(vl,length -2) == '}}'){
    ...
    for (var i = 1; i < arguments.length; i++) {
        var arg = arguments[i];
        if (Helper.isObject(arg) && (arg.isQuery || arg.isDomQuery)) {
            contents.push(arg);
        }
        else if (Helper.isObject(arg) && arg.isDom) {
            contents.push(arg);
        }
            ...
            if (attrs.hasOwnProperty(prop)) {
                ...
                if (Helper.inArray(['tag', 'tagname'], s)) {
                    ...
                    if (val.isDom || val.isDomQuery || _instanceof(val, Element)) {
                        ...
                        var attrObj = Helper.str.convertTextObject({}, val, prop, '-');
        ...
        if (contents.length) {
            if (tagName.toLowerCase() == 'textarea') {
                ...
                for (var i = 0; i < contents.length; i++) {
                    ...
                    else if (Helper.isObject(el) && el.isDom) {
...
* @return {boolean}
*/
function hasEvent(element, event, callback) {
    // trưởng hợp không gửi element nào thì xóa tất cả
    if (!element || (!(element instanceof Element) && !isGlobalOrRoot(element)) || !Helper.isString(event)) {
        return false;
    }
